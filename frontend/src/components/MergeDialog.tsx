...
export const MergeDialog: React.FC<MergeDialogProps> = ({ availableTracks, onClose, onMerge, isMerging }) => {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-300">
            <div className="bg-zinc-900 border border-white/10 rounded-3xl w-full max-w-md shadow-2xl overflow-hidden relative" onClick={e => e.stopPropagation()}>
                        ...
                        {availableTracks.map(track => {
                            ...
                            return (
                                <button
                                    ...
                                    disabled={isMerging}
                                    className={`flex items-center gap-3 w-full p-4 rounded-xl border transition-all text-left ${isSelected
                ...
                {/* Footer */}
                <div className="p-6 pt-2 flex items-center justify-end gap-3 bg-white/[0.02] border-t border-white/5">
                    <button
                        onClick={onClose}
                        disabled={isMerging}
                        className="px-5 py-2.5 rounded-xl font-semibold text-sm text-zinc-400 hover:text-white hover:bg-white/5 transition-all disabled:opacity-50"
                    >
                        Cancel
                    </button>
                    <button
                        ...
                        className="flex items-center gap-2 px-6 py-2.5 rounded-xl font-bold text-sm bg-yellow-500 text-black hover:bg-yellow-400 hover:scale-105 transition-all disabled:opacity-50 disabled:hover:scale-100 disabled:cursor-not-allowed shadow-[0_0_15px_rgba(250,204,21,0.3)]"
                    >
                        {isMerging ? (
                            <>
