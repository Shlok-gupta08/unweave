...
export const Uploader: React.FC<UploaderProps> = ({ onComplete, onJobStarted, onSessionExpired, onClearJob, resumeJobId, hasActiveSession }) => {
        ...
        const poll = async () => {
            try {
                ...
                if (data.status === 'complete' && data.stems) {
                    ...
                    if (data.status === 'cancelled' && onClearJob) {
                        onClearJob();
                    }
                }
            } catch (err) {
